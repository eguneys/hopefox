import { between } from "./distill/attacks"
import { BISHOP, KING, KNIGHT, make_move_from_to, move_c_to_Move, PAWN, piece_c_color_of, piece_c_type_of, PieceTypeC, QUEEN, ROOK } from "./distill/hopefox_c"
import { SquareSet } from "./distill/squareSet"
import { Square } from "./distill/types"
import { PositionMaterializer } from "./pos_materializer"

export type MZ_Future = {
    _mz_views: MZ_Views
    _mz_forks: MZ_Forks
    _mz_typed_forks: MZ_Typed_Forks
    mate: boolean
}

export function mz_future(mz: PositionMaterializer, legal: { from: Square, to: Square }): MZ_Future {

    let move = make_move_from_to(legal.from, legal.to)
    let w2 = mz.inc_add_move(move)
    mz.inc_make_world(w2)
    let _mz_views = mz_views(mz)
    let _mz_forks = mz_forks(_mz_views)
    let _mz_typed_forks = mz_typed_forks(_mz_views, _mz_forks)
    let mate = mz.m.is_checkmate(mz.pos)
    mz.inc_unmake_world(w2)



    return {
        _mz_views,
        _mz_forks,
        _mz_typed_forks,
        mate
    }
}


export type MZ_Typed_Forks = {
    knight_takes_hanging_queen: { from: Square, to: Square }[]
    bishop_forks_king_and_rook: { from: Square, to: Square, king: Square, rook: Square }[]
    uncapturable_knight_check: { from: Square, to: Square, king: Square }[]
    queen_evades_attack: { from: Square, to: Square, attack: Square }[]
    queen_see_king_with_bishop: { queen: Square, king_to: Square, bishop: Square }[]
    queen_see_king_with_bishop_through: { queen: Square, king_to: Square, bishop: Square }[]
    queen_bishop_check: { queen: Square, to: Square, king: Square, bishop: Square }[]
    queen_rook_through_mate: { queen: Square, to: Square, king: Square, rook: Square }[]
    queen_bishop_through_check: { queen: Square, to: Square, king: Square, bishop: Square }[]
    queen_attacks_hanging_knight: { from: Square, to: Square, knight: Square }[]
    rook_attacks_queen: { from: Square, to: Square, queen: Square }[]
    rook_takes_knight: { from: Square, to: Square }[]
    rook_captures_bishop: { from: Square, to: Square }[]
    rook_captures_rook: { from: Square, to: Square }[]
    queen_captures_bishop: { from: Square, to: Square }[]
    queen_captures_queen: { from: Square, to: Square }[]
    rook_captures_queen: { from: Square, to: Square }[]
    bishop_captures_queen: { from: Square, to: Square }[]
    bishop_captures_rook: { from: Square, to: Square }[]
    bishop_captures_hanging_pawn: { from: Square, to: Square }[]
    bishop_captures_hanging_knight: { from: Square, to: Square }[]
    bishop_captures_hanging_bishop: { from: Square, to: Square }[]
    knight_captures_queen: { from: Square, to: Square }[]
    knight_captures_bishop: { from: Square, to: Square }[]
    pawn_captures_queen: { from: Square, to: Square }[]
    pawn_captures_rook: { from: Square, to: Square }[]
    pawn_captures_bishop: { from: Square, to: Square }[]
    pawn_captures_knight: { from: Square, to: Square }[]
    rook_check: { from: Square, to: Square }[]
    blocks_check: { from: Square, to: Square, king: Square, check: Square }[]
    discovered_check: { from: Square, to: Square, king: Square, check: Square }[]
    king_evades_check: { from: Square, to: Square, check: Square }[]
    king_captures_rook: { from: Square, to: Square, rook: Square }[]
    king_captures_bishop: { from: Square, to: Square, bishop: Square }[]
    bishop_pins_pawn: { bishop: Square, pawn: Square, king: Square }[]
    pin_defended_pawn: { pawn: Square, d_pawn: Square }[]
    queen_captures_pin_defended_pawn: { from: Square, to: Square }[]
    queen_captures_hanging_rook: { from: Square, to: Square }[]
    queen_takes_knight: { from: Square, to: Square }[]
    bishop_attack_queen: { from: Square, to: Square, queen: Square }[]
}

export function mz_typed_forks(mz_views: MZ_Views, mz_forks: MZ_Forks): MZ_Typed_Forks {
    


    let queen_see_king_with_rook_through: { queen: Square, king_to: Square, rook: Square }[] = []
    for (let a of mz_forks.attack_double_from_see_through) {
        for (let k of mz_forks.opponent_king_see) {
            if (k.to !== a.to) {
                continue
            }
            for (let q of mz_forks.turn_queens) {
                if (q.from !== a.from_a) {
                    continue
                }
                for (let b of mz_forks.turn_rooks) {
                    if (b.from !== a.from_b) {
                        continue
                    }

                    queen_see_king_with_rook_through.push({ queen: q.from, king_to: a.to, rook: b.from })
                }
            }
        }
    }



    let queen_see_king_with_bishop_through: { queen: Square, king_to: Square, bishop: Square }[] = []
    for (let a of mz_forks.attack_double_from_see_through) {
        for (let k of mz_forks.king_see) {
            if (k.to !== a.to) {
                continue
            }
            for (let q of mz_forks.queens) {
                if (q.from !== a.from_a) {
                    continue
                }
                for (let b of mz_forks.bishops) {
                    if (b.from !== a.from_b) {
                        continue
                    }

                    queen_see_king_with_bishop_through.push({ queen: q.from, king_to: a.to, bishop: b.from })
                }
            }
        }
    }




    let queen_see_king_with_bishop: { queen: Square, king_to: Square, bishop: Square }[] = []
    for (let a of mz_forks.attack_double_from_see) {
        for (let k of mz_forks.king_see) {
            if (k.to !== a.to) {
                continue
            }
            for (let q of mz_forks.queens) {
                if (q.from !== a.from_a) {
                    continue
                }
                for (let b of mz_forks.bishops) {
                    if (b.from !== a.from_b) {
                        continue
                    }

                    queen_see_king_with_bishop.push({ queen: q.from, king_to: a.to, bishop: b.from })
                }
            }
        }
    }

    let checks: { from: Square, king: Square }[] = []
    for (let c of mz_views.attack_see) {
        if (c.to === mz_forks.turn_king.from) {
            checks.push({ from: c.from, king: mz_forks.turn_king.from })
        }
    }

    let checks2: { from: Square, to: Square, king: Square }[] = []
    for (let c of mz_views.attack_see2) {
        if (c.to2 === mz_forks.opponent_king.from) {
            checks2.push({ from: c.from, to: c.to, king: mz_forks.turn_king.from })
        }
    }



    let knight_checks: { from: Square, to: Square, king: Square }[] = []
    for (let c of checks2) {
        for (let k of mz_forks.knights) {

            if (c.from === k.from) {
                knight_checks.push(c)
            }
        }
    }


    let uncapturable_knight_check: { from: Square, to: Square, king: Square }[] = []

    for (let k of knight_checks) {
        for (let b of mz_forks.opponent_non_king_uncapturable) {
            if (b.to !== k.king) {
                continue
            }

            uncapturable_knight_check.push({ from: k.from, to: k.to, king: k.king })
        }
    }



    let queen_bishop_check: { queen: Square, to: Square, king: Square, bishop: Square }[] = []

    for (let q of queen_see_king_with_bishop) {
        for (let b of mz_forks.opponent_non_king_uncapturable) {
            if (b.to !== q.king_to) {
                continue
            }

            queen_bishop_check.push({ queen: q.queen, to: q.king_to, king: mz_forks.opponent_king.from, bishop: q.bishop })
        }
    }


    let bishop_forks_king_and_rook: { from: Square, to: Square, king: Square, rook: Square }[] = []
    for (let f of mz_forks.fork) {
        for (let b of mz_forks.turn_bishops) {
            if (b.from !== f.from) {
                continue
            }

            let king = mz_forks.opponent_king

            if (king.from !== f.fork_a) {
                continue
            }


            for (let rook of mz_forks.opponent_rooks) {
                if (rook.from !== f.fork_b) {
                    continue
                }

                bishop_forks_king_and_rook.push({ from: f.from, to: f.to, king: king.from, rook: rook.from })
            }
        }
    }

    let turn_queen_attack_see2: { from: Square, to: Square, to2: Square }[] = []

    for (let q of mz_forks.turn_queens) {
        for (let a2 of mz_views.attack_see2) {
            if (a2.from === q.from) {
                turn_queen_attack_see2.push(a2)
            }
        }
    }


    let turn_rook_attack_see2: { from: Square, to: Square, to2: Square }[] = []

    for (let q of mz_forks.turn_rooks) {
        for (let a2 of mz_views.attack_see2) {
            if (a2.from === q.from) {
                turn_rook_attack_see2.push(a2)
            }
        }
    }



    let opponent_hanging_queen: { from: Square }[] = []

    for (let q of mz_forks.opponent_queens) {
        for (let h of mz_forks.hanging) {
            if (q.from === h.from) {
                opponent_hanging_queen.push(h)
            }
        }
    }


    let opponent_hanging_pawn: { from: Square }[] = []

    for (let k of mz_forks.opponent_pawns) {
        for (let h of mz_forks.hanging) {
            if (k.from === h.from) {
                opponent_hanging_pawn.push(h)
            }
        }
    }



    let opponent_hanging_bishop: { from: Square }[] = []

    for (let k of mz_forks.opponent_bishops) {
        for (let h of mz_forks.hanging) {
            if (k.from === h.from) {
                opponent_hanging_bishop.push(h)
            }
        }
    }

    let opponent_hanging_rook: { from: Square }[] = []

    for (let k of mz_forks.opponent_rooks) {
        for (let h of mz_forks.hanging) {
            if (k.from === h.from) {
                opponent_hanging_rook.push(h)
            }
        }
    }



    let opponent_hanging_knight: { from: Square }[] = []

    for (let k of mz_forks.opponent_knights) {
        for (let h of mz_forks.hanging) {
            if (k.from === h.from) {
                opponent_hanging_knight.push(h)
            }
        }
    }

    let queen_attacks_hanging_knight: { from: Square, to: Square, knight: Square }[] = []

    for (let h of opponent_hanging_knight) {
        for (let q of turn_queen_attack_see2) {
            if (h.from === q.to2) {
                queen_attacks_hanging_knight.push({ from: q.from, to: q.to, knight: h.from })
            }
        }
    }

    let rook_attacks_queen: { from: Square, to: Square, queen: Square }[] = []

    for (let h of mz_forks.opponent_queens) {
        for (let q of turn_rook_attack_see2) {
            if (h.from === q.to2) {
                rook_attacks_queen.push({ from: q.from, to: q.to, queen: h.from })
            }
        }
    }



    let turn_knight_attack: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_knights) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                turn_knight_attack.push(a)
            }
        }
    }

    let turn_bishop_attack: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_bishops) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                turn_bishop_attack.push(a)
            }
        }
    }



    let turn_rook_attack: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_rooks) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                turn_rook_attack.push(a)
            }
        }
    }



    let turn_queen_attack: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_queens) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                turn_queen_attack.push(a)
            }
        }
    }


    let turn_pawn_attack: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_pawns) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                turn_pawn_attack.push(a)
            }
        }
    }


    let bishop_captures_hanging_pawn: { from: Square, to: Square }[] = []

    for (let h of opponent_hanging_pawn) {
        for (let k of turn_bishop_attack) {
            if (h.from === k.to) {
                bishop_captures_hanging_pawn.push(k)
            }
        }
    }


    let bishop_captures_hanging_knight: { from: Square, to: Square }[] = []

    for (let h of opponent_hanging_knight) {
        for (let k of turn_bishop_attack) {
            if (h.from === k.to) {
                bishop_captures_hanging_knight.push(k)
            }
        }
    }



    let bishop_captures_hanging_bishop: { from: Square, to: Square }[] = []

    for (let h of opponent_hanging_bishop) {
        for (let k of turn_bishop_attack) {
            if (h.from === k.to) {
                bishop_captures_hanging_bishop.push(k)
            }
        }
    }





    let knight_takes_hanging_queen: { from: Square, to: Square }[] = []

    for (let h of opponent_hanging_queen) {
        for (let k of turn_knight_attack) {
            if (h.from === k.to) {
                knight_takes_hanging_queen.push(k)
            }
        }
    }


    let rook_captures_bishop: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_bishops) {
        for (let r of turn_rook_attack) {
            if (r2.from === r.to) {
                rook_captures_bishop.push(r)
            }
        }
    }



    let rook_captures_rook: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_rooks) {
        for (let r of turn_rook_attack) {
            if (r2.from === r.to) {
                rook_captures_rook.push(r)
            }
        }
    }

    let rook_takes_knight: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_knights) {
        for (let r of turn_rook_attack) {
            if (r2.from === r.to) {
                rook_takes_knight.push(r)
            }
        }
    }

    let rook_captures_queen: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_queens) {
        for (let r of turn_rook_attack) {
            if (r2.from === r.to) {
                rook_captures_queen.push(r)
            }
        }
    }


    let bishop_captures_rook: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_rooks) {
        for (let r of turn_bishop_attack) {
            if (r2.from === r.to) {
                bishop_captures_rook.push(r)
            }
        }
    }



    let bishop_captures_queen: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_queens) {
        for (let r of turn_bishop_attack) {
            if (r2.from === r.to) {
                bishop_captures_queen.push(r)
            }
        }
    }

    let knight_captures_bishop: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_bishops) {
        for (let r of turn_knight_attack) {
            if (r2.from === r.to) {
                knight_captures_bishop.push(r)
            }
        }
    }



    let knight_captures_queen: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_queens) {
        for (let r of turn_knight_attack) {
            if (r2.from === r.to) {
                knight_captures_queen.push(r)
            }
        }
    }


    let queen_captures_bishop: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_bishops) {
        for (let r of turn_queen_attack) {
            if (r2.from === r.to) {
                queen_captures_bishop.push(r)
            }
        }
    }




    let queen_captures_queen: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_queens) {
        for (let r of turn_queen_attack) {
            if (r2.from === r.to) {
                queen_captures_queen.push(r)
            }
        }
    }

    let pawn_captures_rook: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_rooks) {
        for (let r of turn_pawn_attack) {
            if (r2.from === r.to) {
                pawn_captures_rook.push(r)
            }
        }
    }


    let pawn_captures_queen: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_queens) {
        for (let r of turn_pawn_attack) {
            if (r2.from === r.to) {
                pawn_captures_queen.push(r)
            }
        }
    }



    let pawn_captures_bishop: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_bishops) {
        for (let r of turn_pawn_attack) {
            if (r2.from === r.to) {
                pawn_captures_bishop.push(r)
            }
        }
    }

    let pawn_captures_knight: { from: Square, to: Square }[] = []
    for (let r2 of mz_forks.opponent_knights) {
        for (let r of turn_pawn_attack) {
            if (r2.from === r.to) {
                pawn_captures_knight.push(r)
            }
        }
    }



    let queen_rook_through_mate: { queen: Square, to: Square, king: Square, rook: Square }[] = []

    for (let q of queen_see_king_with_rook_through) {
        for (let b of mz_forks.opponent_non_king_uncapturable) {
            if (b.to !== q.king_to) {
                continue
            }

            queen_rook_through_mate.push({ queen: q.queen, to: q.king_to, king: mz_forks.opponent_king.from, rook: q.rook })
        }
    }

    let queen_bishop_through_check: { queen: Square, to: Square, king: Square, bishop: Square }[] = []

    for (let q of queen_see_king_with_bishop_through) {
        for (let b of mz_forks.opponent_non_king_uncapturable) {
            if (b.to !== q.king_to) {
                continue
            }

            queen_bishop_through_check.push({ queen: q.queen, to: q.king_to, king: mz_forks.opponent_king.from, bishop: q.bishop })
        }
    }


    let rook_check: { from: Square, to: Square }[] = []


    for (let r of turn_rook_attack_see2) {
        if (r.to2 === mz_forks.opponent_king.from) {
            rook_check.push({ from: r.from, to: r.to })
        }
    }


    let blocks_check: { from: Square, to: Square, king: Square, check: Square }[] = []

    for (let b of mz_forks.block_attack) {
        if (b.a_to === mz_forks.turn_king.from) {
            blocks_check.push({ from: b.from, to: b.to, king: mz_forks.turn_king.from, check: b.a_from })
        }
    }

    let discovered_check: { from: Square, to: Square, king: Square, check: Square }[] = []

    for (let b of mz_forks.unblock_attack) {
        if (b.a_to === mz_forks.opponent_king.from) {
            discovered_check.push({ from: b.from, to: b.to, king: mz_forks.opponent_king.from, check: b.a_from })
        }
    }

    let king_evades_check: { from: Square, to: Square, check: Square }[] = []

    for (let c of checks) {
        for (let e of mz_forks.king_see) {
            if (e.from === c.king) {
                king_evades_check.push({ from: e.from, to: e.to, check: c.from })
            }
        }
    }


    let king_captures_rook: { from: Square, to: Square, rook: Square }[] = []

    for (let r of mz_forks.opponent_rooks) {
        for (let e of mz_forks.king_see) {
            if (e.to === r.from) {
                king_captures_rook.push({ from: e.from, to: e.to, rook: r.from })
            }
        }
    }


    let king_captures_bishop: { from: Square, to: Square, bishop: Square }[] = []

    for (let r of mz_forks.opponent_bishops) {
        for (let e of mz_forks.king_see) {
            if (e.to === r.from) {
                king_captures_bishop.push({ from: e.from, to: e.to, bishop: r.from })
            }
        }
    }



    let queen_evades_attack: { from: Square, to: Square, attack: Square }[] = []


    for (let a of mz_views.attack_see) {
        for (let q of mz_forks.all_see) {
            for (let qq of mz_forks.turn_queens) {
                if (qq.from !== q.from) {
                    continue
                }

                if (a.to === q.from) {
                    queen_evades_attack.push({ from: q.from, to: q.to, attack: a.from })
                }
            }
        }
    }

    let bishop_pins_pawn: { bishop: Square, pawn: Square, king: Square }[] = []

    for (let b of mz_forks.turn_bishops) {
        for (let a of mz_views.attack_see_through) {
            if (a.from !== b.from) {
                continue
            }
            if (a.to !== mz_forks.opponent_king.from) {
                continue
            }

            for (let p of mz_forks.opponent_pawns) {
                if (a.to_through !== p.from) {
                    continue
                }

                bishop_pins_pawn.push({ bishop: b.from, pawn: p.from, king: a.to })
            }
        }
    }


    let pin_defended_pawn: { pawn: Square, d_pawn: Square }[] = []

    for (let d_pawn of bishop_pins_pawn) {
        for (let d of mz_views.defend_see) {
            if (d_pawn.pawn !== d.from) {
                continue
            }
            for (let pawn of mz_forks.opponent_pawns) {
                if (pawn.from !== d.to) {
                    continue
                }

                pin_defended_pawn.push({ pawn: pawn.from, d_pawn: d_pawn.pawn })
            }
        }
    }

    let queen_captures: { from: Square, to: Square }[] = []

    for (let q of mz_forks.turn_queens) {
        for (let a of mz_views.attack_see) {
            if (a.from === q.from) {
                queen_captures.push(a)
            }
        }
    }


    let queen_captures_pin_defended_pawn: { from: Square, to: Square }[] = []

    for (let p of pin_defended_pawn) {
        for (let q of queen_captures) {
            if (p.pawn === q.to) {
                queen_captures_pin_defended_pawn.push(q)
            }
        }
    }


    let queen_captures_hanging_rook: { from: Square, to: Square }[] = []

    for (let h of opponent_hanging_rook) {
        for (let q of queen_captures) {
            if (h.from === q.to) {
                queen_captures_hanging_rook.push(q)
            }
        }
    }


    let queen_takes_knight: { from: Square, to: Square }[] = []

    for (let h of mz_forks.knights) {
        for (let q of queen_captures) {
            if (h.from === q.to) {
                queen_takes_knight.push(q)
            }
        }
    }

    let bishop_attack_queen: { from: Square, to: Square, queen: Square }[] = []

    for (let b of mz_forks.turn_bishops) { 
        for (let q of mz_forks.opponent_queens) {
            for (let ba of mz_views.attack_see2) {
                if (ba.from === b.from && ba.to2 === q.from) {
                    bishop_attack_queen.push({ from: b.from, to: ba.to, queen: q.from })
                }
            }
        }
    }

    return {
        bishop_attack_queen,
        queen_takes_knight,
        queen_captures_hanging_rook,
        queen_captures_pin_defended_pawn,
        pin_defended_pawn,
        bishop_pins_pawn,
        queen_evades_attack,
        uncapturable_knight_check,
        knight_takes_hanging_queen,
        queen_see_king_with_bishop,
        queen_see_king_with_bishop_through,
        bishop_forks_king_and_rook,
        queen_bishop_check,
        queen_rook_through_mate,
        queen_bishop_through_check,
        queen_attacks_hanging_knight,
        rook_attacks_queen,
        rook_captures_bishop,
        rook_captures_rook,
        rook_captures_queen,
        bishop_captures_rook,
        bishop_captures_queen,
        bishop_captures_hanging_pawn,
        bishop_captures_hanging_knight,
        bishop_captures_hanging_bishop,
        knight_captures_queen,
        knight_captures_bishop,
        rook_takes_knight,
        queen_captures_queen,
        queen_captures_bishop,
        pawn_captures_queen,
        pawn_captures_rook,
        pawn_captures_bishop,
        pawn_captures_knight,
        rook_check,
        blocks_check,
        discovered_check,
        king_evades_check,
        king_captures_rook,
        king_captures_bishop,
    }
}



export type MZ_Forks = {
    turn_king: { from: Square }
    turn_pawns: { from: Square }[]
    turn_queens: { from: Square }[]
    turn_bishops: { from: Square }[]
    turn_knights: { from: Square }[]
    turn_rooks: { from: Square }[]
    opponent_pawns: { from: Square }[]
    opponent_knights: { from: Square }[]
    opponent_bishops: { from: Square }[]
    opponent_rooks: { from: Square }[]
    opponent_queens: { from: Square }[]
    opponent_king: { from: Square }
    opponent_king_see: { from: Square, to: Square }[]
    opponent_non_king_uncapturable: { to: Square }[]
    turn_king_capturable: { from: Square, to: Square }[]
    fork: { from: Square, to: Square, fork_a: Square, fork_b: Square }[]
    attack_double_from_see: { from_a: Square, from_b: Square, to: Square }[]
    attack_double_from_see_through: { from_a: Square, from_b: Square, to: Square }[]
    defend_double_from_see: { from_a: Square, from_b: Square, to: Square }[]
    defend_double_from_see2: { from: Square, to_a: Square, from_b: Square, to: Square }[]
    hanging: { from: Square }[]
    king_see: { from: Square, to: Square }[]
    queens: { from: Square }[]
    bishops: { from: Square }[]
    knights: { from: Square }[]
    all_see: { from: Square, to: Square }[]
    block_attack: { from: Square, to: Square, a_from: Square, a_to: Square }[]
    block_defend: { from: Square, to: Square, a_from: Square, a_to: Square }[]
    block_vacant: { from: Square, to: Square, a_from: Square, a_to: Square }[]
    unblock_attack: { from: Square, to: Square, a_from: Square, a_to: Square }[]
    unblock_defend: { from: Square, to: Square, a_from: Square, a_to: Square }[]
    unblock_vacant: { from: Square, to: Square, a_from: Square, a_to: Square }[]
}

export type AttackSee2 = { from: Square, to: Square, to2: Square }

export function mz_forks(mz_views: MZ_Views): MZ_Forks {

    let turn_pawns: { from: Square }[] = []
    let turn_queens: { from: Square }[] = []
    let turn_bishops: { from: Square }[] = []
    let turn_knights: { from: Square }[] = []
    let turn_rooks: { from: Square }[] = []

    let opponent_pawns: { from: Square }[] = []
    let opponent_bishops: { from: Square }[] = []
    let opponent_knights: { from: Square }[] = []
    let opponent_rooks: { from: Square }[] = []
    let opponent_queens: { from: Square }[] = []
    let opponent_king!: { from: Square }
    let opponent_king_see: { from: Square, to: Square }[] = []

    let turn_king!: { from: Square }

    let attack_double_from_see: { from_a: Square, from_b: Square, to: Square }[] = []
    let attack_double_from_see_through: { from_a: Square, from_b: Square, to: Square }[] = []

    let opponent_non_king_uncapturable: { to: Square }[] = []

    let turn_king_capturable: { from: Square, to: Square }[] = []

    let hanging: { from: Square }[] = []

    for (let b of mz_views.turn) {
        if (b.role === KNIGHT) {
            turn_knights.push({ from: b.from })
        } else if (b.role === BISHOP) {
            turn_bishops.push({ from: b.from })
        } else if (b.role === ROOK) {
            turn_rooks.push({ from: b.from })
        } else if (b.role === QUEEN) {
            turn_queens.push({ from: b.from })
        } else if (b.role === KING) {
            turn_king = { from: b.from }
        } else if (b.role === PAWN) {
            turn_pawns.push({ from: b.from})
        }
    }

    for (let b of mz_views.opponent) {
        if (b.role === BISHOP) {
            opponent_bishops.push({ from: b.from })
        } else if (b.role === KNIGHT) {
            opponent_knights.push({ from: b.from })
        } else if (b.role === ROOK) {
            opponent_rooks.push({ from: b.from })
        } else if (b.role === QUEEN) {
            opponent_queens.push({ from: b.from })
        } else if (b.role === KING) {
            opponent_king = { from: b.from }
        } else if (b.role === PAWN) {
            opponent_pawns.push({ from: b.from })
        }
    }

    for (let a of mz_views.attack_see) {
        if (a.from === opponent_king.from) {
            opponent_king_see.push({ from: a.from, to: a.to })
        }
    }
    for (let a of mz_views.defend_see) {
        if (a.from === opponent_king.from) {
            opponent_king_see.push({ from: a.from, to: a.to })
        }
    }
    for (let a of mz_views.vacant_see) {
        if (a.from === opponent_king.from) {
            opponent_king_see.push({ from: a.from, to: a.to })
        }
    }

    let defend_double_from_see: { from_a: Square, from_b: Square, to: Square }[] = []

    for (let a of mz_views.defend_see) {
        for (let b of mz_views.defend_see) {
            if (a.to === b.to) {
                if (a.from !== b.from) {
                    defend_double_from_see.push({ from_a: a.from, from_b: b.from, to: a.to })
                }
            }
        }
    }



    for (let a of mz_views.attack_see) {
        for (let b of mz_views.attack_see) {
            if (a.to === b.to) {
                if (a.from !== b.from) {
                    attack_double_from_see.push({ from_a: a.from, from_b: b.from, to: a.to })
                }
            }
        }
    }

    for (let a of mz_views.attack_see) {
        for (let b of mz_views.attack_see_through) {
            if (a.to === b.to) {
                if (a.from !== b.from) {
                    if (a.from === b.to_through) {
                        attack_double_from_see_through.push({ from_a: a.from, from_b: b.from, to: a.to })
                    }
                }
            }
        }
    }



    for (let o of mz_views.attack_see) {
        if (o.from !== turn_king.from) {
            continue
        }

        turn_king_capturable.push({ from: o.from, to: o.to })
    }

    let defend_or_vacant_see = [...mz_views.defend_see, ...mz_views.vacant_see]

    let non_king_opponent_capturable = SquareSet.empty()
    for (let o of mz_views.opponent) {
        if (o.from === opponent_king.from) {
            continue
        }

        for (let a of defend_or_vacant_see) {
           if (o.from === a.from) {
                non_king_opponent_capturable = non_king_opponent_capturable.set(a.to, true)
            }
        }
    }

    let non_king_opponent_uncapturable = non_king_opponent_capturable.complement()
    for (let sq of non_king_opponent_uncapturable) {
        opponent_non_king_uncapturable.push({ to: sq })
    }


    let fork: { from: Square, to: Square, fork_a: Square, fork_b: Square }[] = []

    let bishop_attacks = new Map<Square, AttackSee2[]>()
    for (let b of mz_views.turn) {
        for (let a of mz_views.attack_see2) {
            if (a.from === b.from) {
                let move = make_move_from_to(a.from, a.to)
                if (!bishop_attacks.has(move)) {
                    bishop_attacks.set(move, [a])
                } else {
                    bishop_attacks.get(move)!.push(a)
                }
            }
        }
    }

    for (let [move, a2s] of bishop_attacks) {
        let { from } = move_c_to_Move(move)
        if (a2s.length === 2) {
            if (a2s[0].to2 === a2s[1].to2) {
                continue
            }
            fork.push({ from, to: a2s[0].to, fork_a: a2s[0].to2, fork_b: a2s[1].to2 })
            fork.push({ from, to: a2s[0].to, fork_a: a2s[1].to2, fork_b: a2s[0].to2 })
        }
        if (a2s.length === 3) {
            if (a2s[0].to2 !== a2s[1].to2) {
                fork.push({ from, to: a2s[0].to, fork_a: a2s[0].to2, fork_b: a2s[1].to2 })
                fork.push({ from, to: a2s[0].to, fork_a: a2s[1].to2, fork_b: a2s[0].to2 })
            }
            if (a2s[0].to2 !== a2s[2].to2) {
                fork.push({ from, to: a2s[0].to, fork_a: a2s[0].to2, fork_b: a2s[2].to2 })
                fork.push({ from, to: a2s[0].to, fork_a: a2s[2].to2, fork_b: a2s[0].to2 })
            }
            if (a2s[2].to2 !== a2s[1].to2) {
                fork.push({ from, to: a2s[2].to, fork_a: a2s[2].to2, fork_b: a2s[1].to2 })
                fork.push({ from, to: a2s[2].to, fork_a: a2s[1].to2, fork_b: a2s[2].to2 })
            }
        }


    }

    let defended_set = SquareSet.empty()
    for (let a of mz_views.defend_see) {
        defended_set = defended_set.set(a.to, true)
    }
    let hanging_set = defended_set.complement()

    for (let a of hanging_set) {
        hanging.push({ from: a })
    }


    let defend_double_from_see2: { from: Square, to_a: Square, from_b: Square, to: Square }[] = []

    for (let a of mz_views.defend_see2) {
        for (let b of mz_views.defend_see) {
            if (a.to2 === b.to) {
                if (a.from !== b.from) {
                    defend_double_from_see2.push({ from: a.from, to_a: a.to, from_b: b.from, to: a.to2 })
                }
            }
        }
    }

    let turn_king_see: { from: Square, to: Square }[] = []

    for (let a of mz_views.attack_see) {
        if (a.from === turn_king.from) {
            turn_king_see.push({ from: a.from, to: a.to })
        }
    }
    for (let a of mz_views.defend_see) {
        if (a.from === turn_king.from) {
            turn_king_see.push({ from: a.from, to: a.to })
        }
    }
    for (let a of mz_views.vacant_see) {
        if (a.from === turn_king.from) {
            turn_king_see.push({ from: a.from, to: a.to })
        }
    }




    let king_see: { from: Square, to: Square }[] = []
    let queens: { from: Square }[] = []
    let bishops: { from: Square }[] = []

    king_see = [...opponent_king_see, ...turn_king_see]

    queens = [...opponent_queens, ...turn_queens]
    bishops = [...opponent_bishops, ...turn_bishops]
    let knights = [...opponent_knights, ...turn_knights]


    let all_see = [...mz_views.attack_see, ...mz_views.defend_see, ...mz_views.vacant_see]

    let block_attack: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []
    let block_defend: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []
    let block_vacant: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []

    for (let a of mz_views.attack_see) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.to)) {
                block_attack.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }


    for (let a of mz_views.defend_see) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.to)) {
                block_defend.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }


    for (let a of mz_views.vacant_see) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.to)) {
                block_vacant.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }


    let unblock_attack: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []
    let unblock_defend: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []
    let unblock_vacant: { from: Square, to: Square, a_from: Square, a_to: Square }[] = []

    for (let a of mz_views.attack_see_through) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.from) && !bb.has(b.to)) {
                unblock_attack.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }

    for (let a of mz_views.defend_see_through) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.from) && !bb.has(b.to)) {
                unblock_defend.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }


    for (let a of mz_views.vacant_see_through) {
        let bb = between(a.from, a.to)
        for (let b of all_see) {
            if (a.from === b.from) continue
            if (bb.has(b.from) && !bb.has(b.to)) {
                unblock_vacant.push({ from: b.from, to: b.to, a_from: a.from, a_to: a.to })
            }
        }
    }




    return {
        fork,
        turn_king,
        turn_pawns,
        turn_queens,
        turn_rooks,
        turn_bishops,
        turn_knights,
        opponent_pawns,
        opponent_knights,
        opponent_bishops,
        opponent_rooks,
        opponent_king,
        opponent_king_see,
        attack_double_from_see,
        defend_double_from_see,
        defend_double_from_see2,
        attack_double_from_see_through,
        opponent_non_king_uncapturable,
        turn_king_capturable,
        hanging,
        opponent_queens,
        king_see,
        queens,
        knights,
        bishops,
        block_attack,
        block_defend,
        block_vacant,
        unblock_attack,
        unblock_defend,
        unblock_vacant,
        all_see
    }
}


export type MZ_Views = {
    turn: { from: Square, role: PieceTypeC }[]
    opponent: { from: Square, role: PieceTypeC }[]
    vacant_see: { from: Square, to: Square }[]
    defend_see: { from: Square, to: Square }[]
    attack_see: { from: Square, to: Square }[]
    vacant_see2: { from: Square, to: Square, to2: Square }[]
    defend_see2: { from: Square, to: Square, to2: Square }[]
    attack_see2: { from: Square, to: Square, to2: Square }[]
    attack_see_through: { from: Square, to: Square, to_through: Square }[]
    defend_see_through: { from: Square, to: Square, to_through: Square }[]
    vacant_see_through: { from: Square, to: Square, to_through: Square }[]
    legals: { from: Square, to: Square }[]
}

export function mz_views(mz: PositionMaterializer): MZ_Views {

    let attack_see_through: { from: Square, to: Square, to_through: Square }[] = []
    let defend_see_through: { from: Square, to: Square, to_through: Square }[] = []
    let vacant_see_through: { from: Square, to: Square, to_through: Square }[] = []

    let vacant_see2: { from: Square, to: Square, to2: Square }[] = []
    let defend_see2: { from: Square, to: Square, to2: Square }[] = []
    let attack_see2: { from: Square, to: Square, to2: Square }[] = []

    let vacant_see: { from: Square, to: Square }[] = []
    let defend_see: { from: Square, to: Square }[] = []
    let attack_see: { from: Square, to: Square }[] = []

    let turn: { from: Square, role: PieceTypeC }[] = []
    let opponent: { from: Square, role: PieceTypeC }[] = []

    let occupied = mz.m.pos_occupied(mz.pos)
    let turn_color = mz.m.pos_turn(mz.pos)

    for (let sq of occupied) {
        let piece = mz.m.get_at(mz.pos, sq)!
        let piece_color = piece_c_color_of(piece)
        let is_turn = piece_color === turn_color
        if (is_turn) {
            turn.push({ from: sq, role: piece_c_type_of(piece) })
        } else {
            opponent.push({ from: sq, role: piece_c_type_of(piece) })
        }


        let aa_occupied = occupied.without(sq)
        let aa = mz.m.pos_attacks(mz.pos, sq)

        for (let a of aa) {
            let piece2 = mz.m.get_at(mz.pos, a)

            let is_vacant = !piece2

            let is_defend = piece2 && piece_c_color_of(piece2) === piece_color

            if (is_vacant) {
                vacant_see.push({ from: sq, to: a })
            } else if (is_defend) {
                defend_see.push({ from: sq, to: a })
            } else {
                attack_see.push({ from: sq, to: a })
            }


            let aa2 = mz.m.attacks(piece, a, aa_occupied)

            for (let a2 of aa2) {
                let piece3 = mz.m.get_at(mz.pos, a2)

                let is_vacant = !piece3

                let is_defend = piece3 && piece_c_color_of(piece3) === piece_color

                if (is_vacant) {
                    vacant_see2.push({ from: sq, to: a, to2: a2 })
                } else if (is_defend) {
                    defend_see2.push({ from: sq, to: a, to2: a2 })
                } else {
                    attack_see2.push({ from: sq, to: a, to2: a2 })
                }
            }


            let aa_through = 
                mz.m.attacks(piece, a, aa_occupied.without(a))
                    .diff(aa)

            for (let a_through of aa_through) {
                let piece3 = mz.m.get_at(mz.pos, a_through)

                let is_vacant = !piece3

                let is_defend = piece3 && piece_c_color_of(piece3) === piece_color

                if (is_vacant) {
                    vacant_see_through.push({ from: sq, to: a_through, to_through: a })
                } else if (is_defend) {
                    defend_see_through.push({ from: sq, to: a_through, to_through: a })
                } else {
                    attack_see_through.push({ from: sq, to: a_through, to_through: a })
                }
            }



        }
    }

    let legals: { from: Square, to: Square }[] = []


    for (let move of mz.inc_generate_legal_moves()) {
        let { from, to } = move_c_to_Move(move)
        legals.push({ from, to })
    }


    return {
        turn,
        opponent,
        vacant_see,
        defend_see,
        attack_see,
        vacant_see2,
        defend_see2,
        attack_see2,
        attack_see_through,
        defend_see_through,
        vacant_see_through,
        legals
    }
}